# Tunnel Ownership Handover Specification

## Purpose

Move ownership of the tunnel process from the runtime that has held it to this
one, in a single operator-held transaction that either completes on evidence or
returns what it took.

The previous owner is not booted out. It keeps the work that is not the tunnel —
selecting the ingress, the Codex fallback's health, the startup and reserve
probes — and is relieved of one job: building and rebuilding the tunnel. Which
runtime holds it is a fact on disk that both read, rather than an assumption in
either, because two runtimes that each believe they own the tunnel is the one
failure neither can recover from.

Completion requires evidence that traffic traversed the tunnel. An interface
that came up is not evidence, and this repository already records elsewhere that
a reachable socket is not qualification.
