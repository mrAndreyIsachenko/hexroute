# pritunl-recovery-ownership Specification

## Purpose
Define the first grant of production authority in this system: one named
service, restarted by the root runtime when the user runtime asks and the root
runtime's own observations agree.

It is the smallest such grant available and is meant to stay that way. The user
runtime holds the credentials and restarts nothing; the root runtime restarts
one service and holds no credential. Neither acts unless an active signed
generation grants the capability, and the answering runtime confirms the fault
for itself rather than accepting the asking runtime's conclusion — a second
opinion, not a relay.

What the two runtimes may conclude, what a refusal must say for itself, and what
an observation that could not be made is allowed to mean are all part of this,
because each of them was once the difference between a fault that was noticed
