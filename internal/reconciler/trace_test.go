...
func TestCanonicalSyntheticTraceCatalogCoversMandatoryTask101Scenarios(t *testing.T) {
	...
	if len(traces) != len(wantScenarios) {
		t.Fatalf("trace count = %d, want %d", len(traces), len(wantScenarios))
	}
	seen := make(map[SyntheticTraceScenario]string, len(traces))
	ackClasses := make(map[AckClass]bool)
	for index, trace := range traces {
		if trace.Scenario != wantScenarios[index] {
			t.Fatalf("trace[%d] scenario = %s, want %s", index, trace.Scenario, wantScenarios[index])
		}
